...
def generate_password(
    ...
    # Крайние случаи: длина <= 0 даёт пустую строку (цикл не выполнится);
    # seed нормализуем, чтобы отрицательные и ноль тоже давали валидный пароль
    # (и чтобы индекс был неотрицательным в любом языке).
    current = abs(seed) % 2147483647
    if current == 0:
        current = 1
