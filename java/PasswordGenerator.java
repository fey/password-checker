...
public final class PasswordGenerator {
    ...
    public static String generatePassword(
        ...
        // Math.abs() перед %, т.к. Java % от отрицательного числа = отрицательный результат
        long current = Math.abs(seed) % 2147483647L;
        if (current == 0) {
            current = 1L;
        }
    ...
    /**
     * Генерирует пароль с набором по умолчанию (строчные + ПРОПИСНЫЕ + цифры).
     *
     * @param length длина пароля
     * @param seed   число-ключ
     * @return сгенерированный пароль
     */
    public static String generatePassword(int length, long seed) {
        return generatePassword(length, seed, true, true, false);
    }
